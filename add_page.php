...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { // Handle the form submission
    ...
    if ($form->validate()) {
        ...
        $q = 'INSERT INTO pages (page_creatorId, page_title, page_content, page_date_added) VALUES (:creatorId, :title, :content, NOW())';
        $stmt = $pdo->prepare($q);
        $r = $stmt->execute(array(':creatorId' => $user->getUserId(), ':title' => $title->getValue(), ':content' => $content->getValue()));
...
include('views/view_add_page.php');
